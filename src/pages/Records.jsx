...
import { Calendar, User, Wrench, TreePine, Clock, ArrowLeft, Trash2, Leaf } from "lucide-react";
...
import RADetailModal, { extractRAData } from "../components/field/RADetailModal";
...
export default function Records() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
                ...
                <div className="space-y-3">
                  {items.map((record) => (
                    ...
                    >
                      ...
                      {(() => {
                        const raData = extractRAData(record.additional_details);
                        if (!raData) return null;
                        const raCode = raData.code || raData.ra_code;
                        return (
                          <button
                            onClick={(e) => { e.stopPropagation(); setSelectedRA(raData); }}
                            className="flex items-center gap-1.5 bg-green-100 text-green-700 border border-green-300 px-2.5 py-1 rounded-full text-xs font-semibold hover:bg-green-200 transition-colors"
                          >
                            <Leaf className="w-3 h-3" />
                            RA: {raCode}
                            {raData.type && <span className="font-normal opacity-70">· {raData.type}</span>}
                          </button>
                        );
                      })()}
                    </div>
      ...
      {selectedRA && (
        <RADetailModal raData={selectedRA} onClose={() => setSelectedRA(null)} />
      )}
