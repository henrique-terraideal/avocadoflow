  ...
  return (
    <div className="bg-muted/30 rounded-xl border border-border p-3 space-y-2">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div>
          <label className={labelClass}>Dose{product.unit ? ` (${product.unit})` : ""}</label>
        ...
        <div>
          <label className={labelClass}>Qtd. Total{product.unit ? ` (${product.unit})` : ""}</label>
      ...
      {qtyPerTank != null && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-1.5 text-xs text-blue-700">
          🧴 <strong>{qtyPerTank}</strong> {product.unit || ""} por tanque
        </div>
      )}
